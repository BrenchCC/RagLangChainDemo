...
detect_model = AutoModelForObjectDetection.from_pretrained(
    "./detect_model/table-transformer-detection",
...
def detect_and_crop_save_table(file_path):
    ...
    # 识别表格
    with torch.no_grad():
        outputs = detect_model(pixel_values)
    # 后处理，得到表格子区域
    id2label = detect_model.config.id2label
    id2label[len(detect_model.config.id2label)] = "no object"
...
def get_embeddings(texts):
    try:
        data = client.embeddings.create(input=texts,model="text-embedding-v2").data
...
if __name__ == "__main__":
    ...
    output_directory_path = pdf2images("./input_pdf/健康档案(含表格01).pdf")
